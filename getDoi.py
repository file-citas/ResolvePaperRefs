...
refs = refex.extractRefs(pdfpath, baseZaKey, args.title)
...
for refid in refkeys:
    refMark = "%s_%d_%s" % (refkeyMark0, refid, refkeyMark1)
    ...
    if refid in ref2ckey.keys():
        ...
        if refid not in nrprinted:
            logging.warning("UPDATE  %d -> \n%s" % (refid, json.dumps(ref2info[refid], indent=2)))
            nrprinted.add(refid)
    else:
        logging.error("Missed Reference: %d" % refid)
        annot = annot.replace(refMark, '[[TODO]](%s)' % refMark)
        if refid in ref2info.keys():
            logging.warning("Missed  %d -> \n%s" % (refid, json.dumps(ref2info[refid], indent=2)))
            nrprinted.add(refid)
